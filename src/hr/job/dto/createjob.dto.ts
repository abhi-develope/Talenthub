...
export class JobBaseDTO extends BaseDTO {
  ...
  @ApiProperty({ type: [String], description: 'List of required skills' })
  @IsArray()
  @ArrayNotEmpty()
  @IsString({ each: true })
  @Expose()
  ...
  @ApiProperty({ enum: EDUCATION_VALUES })
  @IsEnum(EDUCATION_VALUES)
  @Expose()
...
// ---------- Response DTO ----------
export class JobResponseDTO extends JobBaseDTO {
  @ApiProperty()
  @Expose()
...
export class CreateJobResponseDTO extends BaseResponse {
  @ApiProperty({ description: 'Details of the newly created job', type: JobResponseDTO })
  msg: string;
  data: JobResponseDTO;
}
